...
async function publicoConfig({ res, query }) {
  const token = texto(query.t, 64);
  let encuesta = null;
  if (token) {
    const e = await get(`SELECT e.id, e.respondida, s.nombre AS sector, u.nombre AS operador
                     FROM encuestas e
  ...
  json(res, {
    ...
    token: token || null,
    encuesta,
    sectores: await all('SELECT id, nombre FROM sectores WHERE activo = 1 ORDER BY orden, nombre'),
...
async function publicoResponder({ res, req, body }) {
  const valores = {
  ...
  if (token) {
    const e = await get('SELECT * FROM encuestas WHERE token = ?', [token]);
    if (!e) return error(res, 404, 'El enlace de la encuesta no es valido');
    if (e.respondida) return error(res, 409, 'Esta encuesta ya fue respondida. ¡Gracias!');
    await run(`UPDATE encuestas SET respondida = ?, fecha = ?, satisfaccion = ?, resolucion = ?,
    ...
    if (!sectorId || !await get('SELECT id FROM sectores WHERE id = ? AND activo = 1', [sectorId])) {
      return error(res, 400, 'Elegí el sector que te atendió');
    }
    await run(`INSERT INTO encuestas (token, consulta_id, sector_id, canal_id, origen, creada, respondida,
...
const crearLink = requiere('operador', async ({ res, body, usuario }) => {
  const sectorId = enteroONull(body.sector_id);
...
const cargarPorOperador = requiere('operador', async ({ res, body, usuario }) => {
  const consultaId = enteroONull(body.consulta_id);
  const consulta = consultaId ? await get('SELECT * FROM consultas WHERE id = ?', [consultaId]) : null;
  ...
  const p = partesFecha();
  if (consulta && await get('SELECT id FROM encuestas WHERE consulta_id = ? AND respondida IS NOT NULL', [consultaId])) {
