...
const listar = requiere('supervisor', ({ res }) => {
  json(res, all('SELECT id, usuario, nombre, rol, puesto, activo, creado FROM usuarios ORDER BY activo DESC, nombre'));
});

const crear = requiere('admin', ({ res, body }) => {
  ...
  if (get('SELECT id FROM usuarios WHERE usuario = ?', [usuario])) return error(res, 409, 'Ese usuario ya existe');
  const r = run('INSERT INTO usuarios (usuario, nombre, hash, rol, puesto, creado) VALUES (?,?,?,?,?,?)',
  ...
  json(res, get('SELECT id, usuario, nombre, rol, puesto, activo FROM usuarios WHERE id = ?',
    [Number(r.lastInsertRowid)]), 201);
});

const editar = requiere('admin', ({ res, body, params, usuario }) => {
  ...
  run(`UPDATE usuarios SET ${campos.map((c) => `${c} = ?`).join(', ')} WHERE id = ?`,
    [...campos.map((c) => d[c]), id]);
  if (d.activo === 0 || d.hash) run('DELETE FROM sesiones WHERE usuario_id = ?', [id]);
  json(res, get('SELECT id, usuario, nombre, rol, puesto, activo FROM usuarios WHERE id = ?', [id]));
});

/** Cada usuario puede cambiar su propia clave. */
const cambiarClave = requiere('operador', ({ res, body, usuario }) => {
  const fila = get('SELECT * FROM usuarios WHERE id = ?', [usuario.id]);
  ...
  run('UPDATE usuarios SET hash = ? WHERE id = ?', [hashPassword(nueva), usuario.id]);
  json(res, { ok: true });
});
